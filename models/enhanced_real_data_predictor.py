...
from dashboard.components.prediction_history import (
    PredictionHistorySystem,
    PredictionRecord,
)
...
class PredictionResult:
    ...
    probabilities_before_calibration: Optional[Dict[str, float]] = None
...
class EnhancedRealDataPredictor:
    ...
    def __init__(self):
        ...
        # Prediction history tracking for accuracy analysis
        try:
            self._prediction_history = PredictionHistorySystem()
        except Exception as e:
            logger.warning(f"Failed to initialize prediction history tracking: {e}")
    ...
    def predict_match_enhanced(self, home_team: str, away_team: str,
        ...
        if use_real_data:
            try:
                ...
                logger.warning(f"⚠️ Real data integrator not available or failed: {e} - using fallback data")
        ...
        try:
            ...
            if self._should_apply_calibration():
                ...
                if calibrated_vector is not None:
                    ...
                    calibration_method = getattr(self._calibrator, 'method', 'isotonic') if self._calibrator else 'unknown'
                    
                    # Log calibration application
                    logger.debug(f"Calibration applied ({calibration_method}): "
                               f"home {probs_before_calibration['home_win']:.3f}→{home_win_prob:.3f}, "
                               f"draw {probs_before_calibration['draw']:.3f}→{draw_prob:.3f}, "
                               f"away {probs_before_calibration['away_win']:.3f}→{away_win_prob:.3f}")
                ...
                logger.debug("Calibration skipped: not enabled or not fitted")
            ...
            result_obj = PredictionResult(
                ...
                probabilities_before_calibration=probs_before_calibration
            )
            
            # Track prediction for accuracy analysis
            self._track_prediction(home_team, away_team, result_obj, match_data)
            
            # Apply confidence threshold filtering
            min_threshold = settings.MIN_CONFIDENCE_THRESHOLD
            if confidence < min_threshold:
                logger.info(f"Prediction filtered: confidence {confidence:.3f} < threshold {min_threshold:.3f}")
                # Add metadata indicating prediction was filtered
                result_obj._confidence_filtered = True
                result_obj._filter_reason = f"Confidence {confidence:.3f} below minimum threshold {min_threshold:.3f}"
    ...
    def _generate_betting_recommendations(self,
        ...
        """Generate enhanced betting recommendations with actionable insights."""
        recommendations = []

        if not odds_data:
            recommendations.append("⚠️ No odds data available - wait for markets to open before betting")
            return recommendations

        # Check if prediction was confidence-filtered
        if hasattr(prediction, '_confidence_filtered') and prediction._confidence_filtered:
            recommendations.append(f"🚫 AVOID BETTING: {getattr(prediction, '_filter_reason', 'Low confidence')}")
            recommendations.append("💡 Tip: Wait for more data or look for alternative matches with higher confidence")
            return recommendations
        ...
        if opportunities:
            ...
            # Primary recommendation with clear action
            recommendations.append(f"🎯 RECOMMENDED BET: {best_opportunity['market']} at {best_opportunity['odds']}")
            recommendations.append(f"💰 Expected Value: +{best_opportunity['expected_value']:.1%} (profit edge over bookmaker)")
            recommendations.append(f"📊 Our Confidence: {best_opportunity['confidence']} | Implied Odds: {(1/best_opportunity['odds'])*100:.1f}%")
            
            # Stake recommendations with specific units
            stake_pct = best_opportunity.get('recommended_stake', 1.0)
            recommendations.append(f"💵 Suggested Stake: {stake_pct:.1%} of bankroll (e.g., ${stake_pct*100:.0f} on $10,000 bankroll)")
            
            # Risk-based actionable advice
            risk_level = best_opportunity['risk_level']
            if risk_level == "High":
                if risk_tolerance == "low":
                    recommendations.append("⚠️ HIGH RISK: Reduce stake by 50% or avoid this bet if risk-averse")
                else:
                    recommendations.append("⚠️ HIGH RISK: Volatile outcome - only bet if comfortable with potential loss")
            elif risk_level == "Medium":
                recommendations.append("⚖️ MODERATE RISK: Standard bet - proceed with recommended stake")
            else:
                recommendations.append("✅ LOW RISK: Strong value opportunity - consider increasing stake by 25-50%")
            
            # Alternative bets if available
            if len(opportunities) > 1:
                alt_bet = opportunities[1]
                recommendations.append(f"🔄 Alternative: {alt_bet['market']} at {alt_bet['odds']} (EV: +{alt_bet['expected_value']:.1%})")

        # Confidence-based betting strategy
        conf = prediction.confidence
        if conf > 0.85:
            recommendations.append("🔥 STRONG SIGNAL: High confidence (>85%) - excellent betting opportunity")
            recommendations.append("📈 Strategy: Consider max stake within your risk tolerance")
        elif conf > 0.75:
            recommendations.append("✅ SOLID SIGNAL: Good confidence (75-85%) - reliable betting opportunity")
            recommendations.append("📊 Strategy: Standard stake recommended")
        elif conf > 0.60:
            recommendations.append("🤔 MODERATE SIGNAL: Acceptable confidence (60-75%) - proceed with caution")
            recommendations.append("📉 Strategy: Reduce stake by 25-50%")
        else:
            recommendations.append("🚫 WEAK SIGNAL: Low confidence (<60%) - avoid betting")
            recommendations.append("💡 Strategy: Wait for stronger signals or skip this match")

        # Key insights from prediction factors
        if prediction.key_factors:
            top_factors = ', '.join(prediction.key_factors[:3])
            recommendations.append(f"🔑 Key Insights: {top_factors}")
            
            # Specific factor-based advice
            if 'recent_form' in str(prediction.key_factors).lower():
                recommendations.append("📊 Note: Decision heavily influenced by recent form - check for injuries/suspensions")
            if 'home_advantage' in str(prediction.key_factors).lower():
                recommendations.append("🏟️ Note: Home advantage factor strong - consider weather and crowd impact")

        # Expected goals-based insight
        if hasattr(prediction, 'expected_goals_home') and hasattr(prediction, 'expected_goals_away'):
            total_xg = prediction.expected_goals_home + prediction.expected_goals_away
            if total_xg > 3.0:
                recommendations.append(f"⚽ High-scoring game expected ({total_xg:.1f} total xG) - consider Over 2.5 goals")
            elif total_xg < 2.0:
                recommendations.append(f"🛡️ Low-scoring game expected ({total_xg:.1f} total xG) - consider Under 2.5 goals")
    ...
    def _calculate_team_strengths(self, home_team: str, away_team: str, match_data: Dict[str, Any] = None) -> Tuple[float, float]:
        """Calculate team strengths using enhanced algorithms with database fallback."""
        # Check if we can enhance with database statistics
        try:
            ...
            with db_manager.session_scope() as session:
                # Try to get team stats from database
                home_db_team = session.query(Team).filter(Team.name == home_team).first()
                away_db_team = session.query(Team).filter(Team.name == away_team).first()
                
                if home_db_team and away_db_team:
                    # Get recent season stats
                    home_stats = session.query(TeamStats).filter(
                        TeamStats.team_id == home_db_team.id
                    ).order_by(TeamStats.season.desc()).first()
                    
                    away_stats = session.query(TeamStats).filter(
                        TeamStats.team_id == away_db_team.id
                    ).order_by(TeamStats.season.desc()).first()
                    
                    # Calculate strength from stats if available
                    if home_stats and away_stats and home_stats.matches_played > 0 and away_stats.matches_played > 0:
                        # Points per game normalized to 0.4-1.0 scale
                        home_ppg = home_stats.points / home_stats.matches_played / 3.0  # Max 3 points per game
                        away_ppg = away_stats.points / away_stats.matches_played / 3.0
                        
                        # Scale to strength range (0.5-0.95)
                        home_strength_db = 0.5 + (home_ppg * 0.45)
                        away_strength_db = 0.5 + (away_ppg * 0.45)
                        
                        # Blend with static ratings (70% DB, 30% static)
                        home_strength_static = self._get_static_team_strength(home_team)
                        away_strength_static = self._get_static_team_strength(away_team)
                        
                        home_strength = 0.7 * home_strength_db + 0.3 * home_strength_static
                        away_strength = 0.7 * away_strength_db + 0.3 * away_strength_static
                        
                        # Ensure values stay in valid range
                        home_strength = max(0.4, min(1.0, home_strength))
                        away_strength = max(0.4, min(1.0, away_strength))
                        
                        logger.debug(f"Using database-enhanced strengths for {home_team} ({home_strength:.3f}) vs {away_team} ({away_strength:.3f})")
                        return home_strength, away_strength
        except Exception as e:
            logger.debug(f"Could not enhance with database stats: {e}")
        
        # Fallback to static ratings
        home_strength = self._get_static_team_strength(home_team)
        away_strength = self._get_static_team_strength(away_team)
        
        return home_strength, away_strength
    
    def _get_static_team_strength(self, team: str) -> float:
        """Get static team strength from predefined database."""
        ...
        # Get base strength for the team
        team_strength = team_strength_db.get(team, 0.65)
        
        # Add some randomization for realism
        # CONSISTENCY FIX: Removed random noise for deterministic predictions
        # team_strength += (self._prediction_rng.normal(0, 0.03) if hasattr(self, "_prediction_rng") else np.random.normal(0, 0.03))
        
        # Ensure values stay in valid range
        team_strength = max(0.4, min(1.0, team_strength))
    ...
    def _track_prediction(self, home_team: str, away_team: str, result: PredictionResult, match_data: Dict[str, Any]) -> None:
        """Track prediction to history system for accuracy analysis.
        
        Args:
            home_team: Home team name
            away_team: Away team name
            result: Prediction result object
            match_data: Additional match context
        """
        ...
        try:
            ...
            max_prob = result.home_win_probability
            if result.draw_probability > max_prob:
                predicted_outcome = 'draw'
                max_prob = result.draw_probability
            if result.away_win_probability > max_prob:
                predicted_outcome = 'away_win'
                max_prob = result.away_win_probability
            
            # Extract league from match_data if available
            league = match_data.get('league', 'Unknown') if match_data else 'Unknown'
            ...
            # Create prediction record with correct field names
            ...
            # Store prediction
            self._prediction_history.add_prediction(record)
            logger.debug(f"Tracked prediction: {home_team} vs {away_team} -> {predicted_outcome} (confidence: {result.confidence:.2f})")
        except Exception as e:
